...
PyObject* PY_LAMMPS_GNN_NS::new_dimenet(double _cutoff, int _hidden_channels, 
	...
	PyObject* net_class = PyObject_GetAttrString(module, "DimeNet");
	if(net_class == nullptr){
		PyErr_Print();
		printf("[lammpsGNN]: error loading class\n");
	}
...
*/
double PY_LAMMPS_GNN_NS::compute_energy(PyObject* net, int n, long* z,
	double** x, double cellx, double celly, double cellz, double** f){
	...
	for(i=0; i<3; i++){
		...
		for(j=0; j<3; j++){
			if(j == i){
				py_v = PyFloat_FromDouble(v);
			} else {
				py_v = PyFloat_FromDouble(0.0);
			}
			PyList_SetItem(py_sublist, j, py_v);
		}
	...
	PyObject* py_res = PyObject_CallMethod(net, "compute_energy_from_lammps", "OOO", py_z, py_x, py_cell);
	if(py_res == nullptr){
		PyErr_Print();
		printf("[lammpsGNN]: error calling compute_energy_from_lammps\n");
	}
